...
export default function ScrollIndicator({selectedPage, setSelectedPage}:any){
    const selectedStyles = 
        `relative bg-yellow before:absolute before:w-6 
    ...
    return(
        <div className="flex flex-col gap-6 fixed top-[60%] right-7">
            ...
            />
            <AnchorLink
                className={`${selectedPage==="references" ? selectedStyles : "bg-dark-grey"} 
                w-3 h-3 rounded-full`}
                href={`#references`}
                onClick={()=> setSelectedPage("references")}
            />
            <AnchorLink
